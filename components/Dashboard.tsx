...
import { AnalysisResult, TranscriptItem, SentimentPoint } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ data, audioFile, onReset }) => {
  ...
  const handleCopySummary = async () => {
    const text = `
Archival Designs Call QA Report
--------------------------------
Summary: ${data.coaching.summary}

Strengths:
${data.coaching.strengths.map(s => `- ${s}`).join('\n')}

Opportunities:
${data.coaching.missedOpportunities.map(o => `- ${o}`).join('\n')}
    `.trim();
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className={`text-sm font-medium px-4 py-2 rounded-md transition-all border ${
                copying 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-full">
          ...
          <div className="lg:col-span-7 flex flex-col gap-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <CoachingCard type="strength" title="SOP Adherence" points={data.coaching.strengths} />
              <CoachingCard type="weakness" title="SOP Deviations" points={data.coaching.missedOpportunities} />
            </div>
          </div>
        </div>
      </main>
  ...
  return (
    <div className={`flex flex-col ${isRep ? 'items-end' : 'items-start'}`}>
      ...
      <div className={`max-w-[90%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${
        isRep ? 'bg-orange-600 text-white rounded-tr-sm' : 'bg-slate-100 text-slate-800 rounded-tl-sm'
      }`}>
        {item.text}
      </div>
    </div>
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <AreaChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
        ...
        <XAxis dataKey="time" tick={{fontSize: 10, fill: '#64748b'}} tickLine={false} axisLine={false} minTickGap={20} />
...
const CoachingCard: React.FC<{ type: 'strength' | 'weakness', title: string, points: string[] }> = ({ type, title, points }) => {
  ...
  return (
    <div className={`rounded-xl border p-5 ${isStrength ? 'bg-emerald-50/50 border-emerald-100' : 'bg-red-50/50 border-red-100'}`}>
      <div className="flex items-center gap-2 mb-4">
        {isStrength ? <CheckCircleIcon className="w-5 h-5 text-emerald-600" /> : <AlertCircleIcon className="w-5 h-5 text-red-600" />}
      ...
      <ul className="space-y-3">
        {points.map((point, idx) => (
          <li key={idx} className="flex gap-3 text-sm text-slate-700 leading-snug">
            <span className={`mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0 ${isStrength ? 'bg-emerald-500' : 'bg-red-500'}`} />
            <span>{point}</span>
          </li>
        ))}
      </ul>
...
const AudioPlayer: React.FC<{ file: File }> = ({ file }) => {
  ...
  return (
    <div className="w-full flex items-center gap-4">
      <audio ref={audioRef} src={url || ''} onTimeUpdate={handleTimeUpdate} onLoadedMetadata={() => setDuration(audioRef.current?.duration || 0)} onEnded={() => setIsPlaying(false)} />
      <button onClick={togglePlay} className="w-10 h-10 flex items-center justify-center bg-slate-900 text-white rounded-full">
        {isPlaying ? <PauseIcon className="w-4 h-4" /> : <PlayIcon className="w-4 h-4 ml-0.5" />}
      </button>
      <div className="flex-1">
        <input type="range" min="0" max="100" step="0.1" value={progress || 0} onChange={(e) => {
          const newTime = (parseFloat(e.target.value) / 100) * duration;
          if (audioRef.current) audioRef.current.currentTime = newTime;
        }} className="w-full h-1.5 accent-orange-600" />
